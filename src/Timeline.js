...
const getSkillColor = (type) => {
  ...
  if (t.includes("cc") || t.includes("crowd") || t.includes("cost")) return 'linear-gradient(to bottom, #8e24aa, #6a1b9a)';
  return 'linear-gradient(to bottom, #757575, #616161)';
};
...
const Timeline = ({ 
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e) => {
      ...
      let newStartTime = Math.ceil((dragState.originalStartTime + timeDelta) * 30) / 30;
  ...
  return (
    <div style={{ border: '1px solid #444', background: '#222', userSelect: 'none', display: 'flex', flexDirection: 'column', borderRadius:'4px' }}>
        ...
        <div style={{ width: `${totalWidth}px`, height: '100%', position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', top: `${HEADER_HEIGHT}px`, left: 0, right: 0, bottom: 0 }}>
             ...
             {events.map((event, index) => {
               ...
               // Resolve target name for tooltip if available
               const targetName = event.targetId ? activeTeam.find(s => s.id === event.targetId)?.name : null;
